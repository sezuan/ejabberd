...
handle_call({get_services, JID, #request{host = ReqHost,
	    ...
	    #state{host = Host,
		   ...
		   secret = Secret,
		   ttl = TTL} = State) ->
    ?DEBUG("Getting STUN/TURN service list for ~s", [jid:encode(JID)]),
...
handle_cast({reload, NewOpts, _OldOpts}, #state{host = Host} = State) ->
    ?DEBUG("Reloading STUN/TURN discovery configuration for ~s", [Host]),
...
terminate(Reason, #state{host = Host}) ->
    ?DEBUG("Stopping STUN/TURN discovery process for ~s: ~p",
	   [Host, Reason]),
...
code_change(_OldVsn, #state{host = Host} = State, _Extra) ->
    ?DEBUG("Updating STUN/TURN discovery process for ~s", [Host]),
    {ok, State}.
...
disco_local_features({result, OtherFeatures} = Acc, From,
	...
	allow ->
	    ?DEBUG("Announcing feature to ~s", [jid:encode(From)]),
	    {result, [?NS_EXTDISCO_2 | OtherFeatures]};
	deny ->
	    ?DEBUG("Not announcing feature to ~s", [jid:encode(From)]),
	    Acc
    end;
...
stun_get_password(<<>>, Username, _Realm) ->
	...
	[Expiration, <<_UserHash:8/binary, HostHash:8/binary>>] ->
		...
		ExpireTime ->
		    ...
		    ?DEBUG("Non-numeric expiration field: ~s", [Username]),
		    <<>>
	    end;
	_ ->
	    ?DEBUG("Not an ephemeral username: ~s", [Username]),
	    <<>>
    end;
...
parse_listener({{Port, _Addr, Transport}, ?STUN_MODULE, Opts}) ->
    Addr = get_listener_address(Opts),
...
-spec get_listener_address(map()) -> inet:ip_address().
get_listener_address(#{ip := {  0,   0, 0, 0}, turn_ip := TurnIP}) -> TurnIP;
get_listener_address(#{ip := {127,   _, _, _}, turn_ip := TurnIP}) -> TurnIP;
get_listener_address(#{ip := { 10,   _, _, _}, turn_ip := TurnIP}) -> TurnIP;
get_listener_address(#{ip := {172,  16, _, _}, turn_ip := TurnIP}) -> TurnIP;
get_listener_address(#{ip := {192, 168, _, _}, turn_ip := TurnIP}) -> TurnIP;
get_listener_address(#{ip := IP}) -> IP.
